...
class NaturalPG(PolicyGradient):
    ...
    def _fvp(self, params: torch.Tensor) -> torch.Tensor:
        ...
        if isinstance(p_dist, tuple) and isinstance(q_dist, tuple):
            cont_kl = torch.distributions.kl.kl_divergence(p_dist[0], q_dist[0]).mean()
            disc_kl = torch.distributions.kl.kl_divergence(p_dist[1], q_dist[1]).mean()
            kl = cont_kl + disc_kl
        else:

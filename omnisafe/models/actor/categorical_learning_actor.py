...
# ==============================================================================
"""Implementation of CategoricalLearningActor."""
...
class CategoricalLearningActor(CategoricalActor):
    """Implementation of CategoricalLearningActor.

    CategoricalLearningActor is a categorical actor for discrete action spaces. It outputs
    logits for each possible action, which are then converted to probabilities using softmax.
    It is used in on-policy algorithms such as ``PPO``, ``TRPO`` for discrete action spaces.

    Args:
        obs_space (OmnisafeSpace): Observation space.
        act_space (OmnisafeSpace): Action space (must be Discrete).
    ...
    def __init__(
        ...
        """Initialize an instance of :class:`CategoricalLearningActor`."""
        super().__init__(obs_space, act_space, hidden_sizes, activation, weight_initialization_mode)

        # For discrete action spaces, act_space.n gives the number of actions
        if hasattr(act_space, 'n'):
            self._act_dim = act_space.n
        else:
            raise ValueError("Action space must be Discrete with attribute 'n'")

        self.logits_net: nn.Module = build_mlp_network(
            sizes=[self._obs_dim, *self._hidden_sizes, self._act_dim],
            activation=activation,
            weight_initialization_mode=weight_initialization_mode,
            output_activation='identity',  # No activation, raw logits
            norm=True,
        )
    ...
    def _distribution(self, obs: torch.Tensor) -> Categorical:
        ...
        Returns:
            The categorical distribution over discrete actions.
        """
        logits = self.logits_net(obs) / self._temperature
    ...
    def predict(self, obs: torch.Tensor, deterministic: bool = False) -> torch.Tensor:
        ...
        - If ``deterministic`` is ``True``, the predicted action is the mode (argmax) of the distribution.
        ...
        Returns:
            The mode of the distribution if deterministic is True, otherwise the sampled action.
        """
    ...
    def log_prob(self, act: torch.Tensor) -> torch.Tensor:
        ...
        # For categorical distribution, act should be integer indices

...
# ==============================================================================
"""Implementation of MultiHeadDoubleActorDiscrete with independent networks."""
...
class MultiHeadDoubleActorDiscrete(Actor):
    """MultiHeadDoubleActorDiscrete with completely independent networks for discrete env actions and binary sensor mask.

    Unlike MultiHeadDiscreteActor which uses a shared backbone, this actor uses two completely separate networks:
    - One network for environment actions (discrete, Categorical distribution)
    ...
    Args:
        ...
        disc_env_act_space: Discrete action space for environment actions (Discrete).
    ...
    def __init__(
        ...
        """Initialize the MultiHeadDoubleActorDiscrete with independent networks."""
        # Pass disc_env_act_space as act_space to base class
        super().__init__(obs_space, disc_env_act_space, env_hidden_sizes, activation, weight_initialization_mode)

        # Environment action network (discrete, like CategoricalLearningActor)
        self.num_env_actions = disc_env_act_space.n
        self.env_net = build_mlp_network(
            sizes=[self._obs_dim, *env_hidden_sizes, self.num_env_actions],
        ...
        self.mask_net = build_mlp_network(
            sizes=[self._obs_dim, *mask_hidden_sizes, self.num_sensors],
            activation=activation,
            weight_initialization_mode=weight_initialization_mode,
    ...
    def _distribution(self, obs):
        ...
        """
        # Environment action distribution (Categorical)
        env_logits = self.env_net(obs) / self._temperature

...
class OnPolicyAdapter(OnlineAdapter):
    ...
    def _log_value(
        ...
        # Track sensor activations from info dictionary
        # Assuming info contains sensor_mask or sensor_active key
        # Check if we're dealing with a vectorized environment or a single environment
        if self._env.use_all_obs:
            return
        if self._env.num_envs > 1:
            # For vectorized environments, iterate through each environment's mask
            try:
                for idx, mask in enumerate(info['sensor_mask']):
                    # Process the mask as a flat array
                    for sensor_idx, is_active in enumerate(mask):
                        sensor_name = f'sensor_{sensor_idx}'
                        if sensor_name not in self._ep_sensor_activations[idx]:
                            self._ep_sensor_activations[idx][sensor_name] = 0.0
                        self._ep_sensor_activations[idx][sensor_name] += float(is_active)
            except KeyError:
                for idx, mask in enumerate(info['final_info']['sensor_mask']):
                    for sensor_idx, is_active in enumerate(mask):
                        sensor_name = f'sensor_{sensor_idx}'
                        if sensor_name not in self._ep_sensor_activations[idx]:
                            self._ep_sensor_activations[idx][sensor_name] = 0.0
            ...
            # For a single environment, process the mask directly
            idx = 0  # Single environment index
            try:
                mask = info['sensor_mask']
            except KeyError:
                mask = info['final_info']['sensor_mask']
            for sensor_idx, is_active in enumerate(mask):
                sensor_name = f'sensor_{sensor_idx}'
                if sensor_name not in self._ep_sensor_activations[idx]:
                    self._ep_sensor_activations[idx][sensor_name] = 0.0
    ...
    def _log_metrics(self, logger: Logger, idx: int) -> None:
        ...
        if self._env.use_all_obs:
            return
        # Log sensor activation percentages
        if self._ep_len[idx] > 0:
            for sensor_name, count in self._ep_sensor_activations[idx].items():
                activation_percentage = count / self._ep_len[idx]
                logger.store({f'Metrics/EpActivation{sensor_name.capitalize()}': activation_percentage})
    ...
    def _reset_log(self, idx: int | None = None) -> None:
        ...
        if idx is None:
            ...
            # Initialize sensor activations tracking for each environment
            self._ep_sensor_activations = [{} for _ in range(self._env.num_envs)]

        else:
            self._ep_ret[idx] = 0.0
            self._ep_cost[idx] = 0.0
            self._ep_len[idx] = 0.0
            # Reset sensor activations for this specific environment
            self._ep_sensor_activations[idx] = {}
...
class RliableJSONWriter:
    """
    Collects evaluation results in the form:
      { "<bucket>": { "<env>_<algo>": [float, float, ...] } }
    and writes/merges them into a JSON file.
    """
    def __init__(self, path: str):
        self.path = path
        self.buffer: Dict[str, Dict[str, List[float]]] = {}

    def add(self, bucket: str, key: str, values: List[float]):
        if bucket not in self.buffer:
            self.buffer[bucket] = {}
        if key not in self.buffer[bucket]:
            self.buffer[bucket][key] = []
        # extend with this evaluation’s episode returns
        self.buffer[bucket][key].extend([float(v) for v in values])

    def save(self):
        # merge with existing file (if any)
        on_disk: Dict[str, Dict[str, List[float]]] = {}
        if os.path.isfile(self.path):
            try:
                with open(self.path, "r") as f:
                    on_disk = json.load(f)
            except Exception:
                # corrupt or empty file—start fresh but don’t crash training
                on_disk = {}

        # merge buffer into on_disk
        for bucket, kv in self.buffer.items():
            if bucket not in on_disk:
                on_disk[bucket] = {}
            for k, vals in kv.items():
                if k not in on_disk[bucket]:
                    on_disk[bucket][k] = []
                on_disk[bucket][k].extend(vals)
...
class Logger:  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(  # pylint: disable=too-many-arguments,too-many-locals
        ...
        rliable_json_path: Optional[str] = None,
    ) -> None:
        ...
        if self._use_wandb and self._maste_proc:  # pragma: no cover
            ...
            if models is not None:
                for model in models:
                    wandb.watch(model)
        self._rliable_writer: Optional[RliableJSONWriter] = None
        if rliable_json_path:
            self._rliable_writer = RliableJSONWriter(rliable_json_path)
    ...
    def add_rliable_eval(self, bucket: str, env_algo_key: str, episode_returns: List[float]):
        if self._rliable_writer is not None:
            self._rliable_writer.add(bucket, env_algo_key, episode_returns)

    def flush_rliable_eval(self):
        if self._rliable_writer is not None:
            self._rliable_writer.save()


    def wandb_log(self, msg: dict) -> None:
        """Log the message to wandb.

        Args:
            msg (str): The message to be logged.
        """
        if self._maste_proc and self._use_wandb:
            wandb.log(msg)

...
SPAR environments extend standard Gym/Gymnasium environments by:
...
### Purpose
Abstract base class that implements core SPAR functionality. All environment-specific wrappers inherit from this class.
...
SPAR environments implement the Constrained MDP interface required by safe RL algorithms:
...
Original highway-env observations are flattened 2D arrays (vehicles × features). SPAR decomposes them into:
...
from spar_envs import BudgetAwareHighway
...
Robosuite observations are dictionaries with multiple keys. SPAR flattens and groups them:
...
```python
import gymnasium as gym
from spar_envs import BudgetAwareRobosuite

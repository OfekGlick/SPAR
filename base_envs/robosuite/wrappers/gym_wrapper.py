...
from base_envs.robosuite.wrappers import Wrapper
...
class GymWrapper(Wrapper, gym.Env):
    ...
    def __init__(self, env, keys=None, flatten_obs=True, key_groups=None):
        # Run super method
    ...
    def _flatten_obs(self, obs_dict, verbose=False):
        ...
        # If key_groups provided, iterate by groups to ensure contiguous modality ranges
        if self.key_groups is not None:
            for group_name, group_keys in self.key_groups.items():
                for key in group_keys:
                    if key not in obs_dict:
                        continue

                    # Special handling for camera images
                    if 'image' in key:
                        if key in self.keys:
                            img = obs_dict[key]
                            ...
                            if img.ndim == 3 and img.shape[-1] == 3:
                                ...
                                # Already grayscale or unexpected format
                                gray = img
                            # Normalize from [0, 255] to [0, 1]
                            gray_normalized = gray.astype(np.float32) / 255.0
                            if verbose:
                                print("adding camera key: {}, shape: {} -> grayscale: {}".format(
                                    key, img.shape, gray_normalized.shape))
                            ob_lst.append(gray_normalized.flatten())
                        continue
            ...
            # Default behavior: iterate through self.keys
